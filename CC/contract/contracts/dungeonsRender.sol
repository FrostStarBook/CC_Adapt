...
contract dungeonsRender {
    ...
    function draw(IDungeons.Dungeon memory dungeon, uint8[] memory x, uint8[] memory y, uint8[] memory entityData)
        external
        view
    ...
    function chunkDungeon(IDungeons.Dungeon memory dungeon, RenderHelper memory helper)
        internal
        view
        returns (string memory)
    {
        ...
        for (uint256 y = 0; y < dungeon.size; y++) {
            ...
            for (uint256 x = 0; x < dungeon.size; x++) {
                if (
                    getBit(helper.layout, helper.counter) == 1 && helper.counter > 0
                        && getBit(helper.layout, helper.counter - 1) == 0
                ) {
                    ...
                    getBit(helper.layout, helper.counter) == 0 && helper.counter > 0
                        && getBit(helper.layout, helper.counter - 1) == 1
                ) {
    ...
    function drawTile(string memory row, uint256 x, uint256 y, uint256 width, uint256 pixel, string memory color)
        internal
        pure
        returns (string memory)
    {
     ...
     */
    function tokenURI(uint256 tokenId, IDungeons.Dungeon memory dungeon, uint256[] memory entities)
        public
        view
        returns (string memory)
    {
